...
Description: <strong>-x- PLEASE NOTE: When updating, you will have to RE-SAVE the settings -x-</strong><br>PayU India supports both PayUmoney and PayUbiz.
Version: 2.0.2
Author: _KDC-Labs
	...
	class WC_Gateway_PayUindia extends WC_Payment_Gateway {
			...
			$this->form_fields = array(
    			...
    			'key_secret' => array(
      			...
      			'show_logo' => array(
					...
					'type' 			=> 'select',
					'label' 		=> __('Enable PayUindia TEST Transactions.', 'woo_payuindia'),
        ...
        function check_payuindia_response(){
			...
			if( isset($_REQUEST['txnid']) && isset($_REQUEST['mihpayid']) ){
				...
				if ( version_compare(WOOCOMMERCE_VERSION, '2.0.0', '>=' ) ) {
					$redirect_url = add_query_arg( array('msg'=> urlencode($this->msg['message']), 'type'=>$this->msg['class']), $redirect_url ); //update for woocommerce >2.0
				}
